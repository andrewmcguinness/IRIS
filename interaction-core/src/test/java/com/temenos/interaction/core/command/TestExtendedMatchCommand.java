...
/* The point of this is to create a MatchCommand with different behaviour
 */
public class TestExtendedMatchCommand {
				...
				protected boolean simpleExpression(String op, String left, String right) {
					if ( "<".equals(op) ) {
						try {
							...
							//LOGGER.warn("Numeric values expected, got " + left + "," + right);
							return false;
						}
					} else
						return super.simpleExpression(op, left, right);
				}
			};
	...
	@Test
	public void testNumericLess() throws InteractionException {
		setExpression("{Number} < 100");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.SUCCESS, result);
	}

	@Test
	public void testNumericLess2() throws InteractionException {
		setExpression("{Number} < 99");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.SUCCESS, result);
	}

	@Test
	public void testNumericLess3() throws InteractionException {
		setExpression("{Number} < 99.3");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.SUCCESS, result);
	}

	@Test
	public void testNumericLessFail() throws InteractionException {
		setExpression("{Number} < 8.1");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.FAILURE, result);
	}

	@Test
	public void testNumericLessInvalid() throws InteractionException {
		setExpression("{Number} < wibble");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.FAILURE, result);
	}

	@Test
	public void testNumericLessInvalid2() throws InteractionException {
		setExpression("wibble < {Number}");
		InteractionCommand.Result result = cmd.execute(ctx);
		assertEquals(InteractionCommand.Result.FAILURE, result);
	}
